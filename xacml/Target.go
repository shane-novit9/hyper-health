...
//The target is the area of a request or policy defining decision requests
//identified by definitions for resource, subject, and action that a rule, policy,
...
func evaluateMatchBody(matchInterface interface{}, request Request) string {
	...
	if err != nil {
		log.Printf("attributeDesignator not ok %v", err)
		return targetIndeterminate
	}
	var result bool
	if attributeSeq == "0" {
