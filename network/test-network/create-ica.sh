...
set -x
./fabric-ca-client enroll -m admin -u http://adminCA:adminpw@ca.root:7071 
{ set +x; } 2>/dev/null
...
./fabric-ca-client register --id.name ica.dummyOrg --id.type client \
 ...
 --csr.cn ica.dummyOrg -m ica.dummyOrg --id.attrs  '"hf.IntermediateCA=true"' -u http://ca.root:7071
{ set +x; } 2>/dev/null
